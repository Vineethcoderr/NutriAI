...
st.markdown(
    "<h1 style='text-align: center;'>🥘 NutriAi - AI Meal Analyzer</h1>",
...
with analyze_tab:
    ...
    with col1:
        ...
        uploaded_file = st.file_uploader(
            "📤 Upload your delicious meal photo...", type=["jpg", "jpeg", "png"]
        )
        # Additional prompt for food preparation details
        prep_prompt = st.text_input(
            "Tell us how your meal was prepared (e.g., grilled, fried, steamed) for a more detailed analysis:"
        )
        # Analyze Button
...
with plan_tab:
    ...
    diet_preferences = st.text_input(
        "Enter your food preferences (e.g., vegetarian, low-carb, gluten-free, etc.):"
    )
    if st.button("Generate Meal Plan"):
        with st.spinner("Creating your personalized meal plan..."):
            try:
                ...
                def generate_pdf(meal_plan_text):
                    pdf = FPDF()
                    pdf.add_page()
                    ...
                    pdf_path = "personalized_meal_plan.pdf"
                    pdf.output(pdf_path)
                    return pdf_path
                ...
                pdf_file_path = generate_pdf(meal_plan_response.text)
                with open(pdf_file_path, "rb") as f:
                    st.download_button(
                        label="Download Meal Plan 📄 (PDF)",
                        data=f,
                        file_name="personalized_meal_plan.pdf",
